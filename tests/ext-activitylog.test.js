...
describe('filtering logs from the logs table', () => {
  describe('filtering logs with checking/unchecking checkboxes', () => {
    ...
    test('show/hide logs associated with extension id that is checked/unchecked from filter option', async () => {
      ...
      expect(window.customElements.get('filter-timestamp')).toBe(
        FilterTimestamp
      );
  ...
  describe('filtering logs with the help of context menu', () => {
    ...
    test('filter by timestamp can be applied with the help of context menu', () => {
      const logs = [
        {
          /* renders the 1st row in the table */
          id: 'id1@test',
  ...
  test('tabId in search param should filter logs by the given tab id', () => {
    ...
    history.replaceState(
      null,
      null,
      `${location.href.split('?')[0]}?tabId=982`
    );
...
test('clicking the clear logs button should remove all logs from logs table', async () => {
  const logs = [
    {
      /* renders the 1st row in the table */
      id: 'id1@test',
...
test('timestamp should be correctly formatted in the logs table', () => {
  const logs = [
    {
      /* renders the 1st row in table */
      id: 'id1@test',
...
test('log details should be displayed when a log is clicked from the logs table', async () => {
  const logs = [
    {
      /* renders the 1st row in table */
      id: 'id1@test',
...
test('viewtype cell should be empty in the logs table when viewtype of a log is undefined', async () => {
  const logs = [
    {
      /* renders the 1st row in table */
      /* viewType = undefined */
...
test('log detail should be toggled when a table row/cell is clicked', async () => {
  const logs = [
    {
      /* renders the 1st row in table */
      id: 'id1@test',
...
test('activitylog page should be initialized with the loaded file\'s logs when the "file" search param is present', async () => {
  const logs = [
    {
      /* renders the 1st row in table */
      id: 'id1@test',
...
test('filter options should be set from URL params initially', () => {
  const addListener = jest.fn();
  const sendMessage = jest.fn();
  const connect = jest.fn();
...
test('handleEvent method of Controller class should display error message if wrong event is found', () => {
  const addListener = jest.fn();
  const sendMessage = jest.fn();
  const connect = jest.fn();
